...
#define ELF_MAP_PROTECT_ERROR (-1)
    ...
    void do_relocations(elf_file &elf, size_t the_load_bias) const {
        for (size_t i = 0; i < elf.number_of_program_headers(); i++) {
            typename elf_file::segment& segment = elf.parsed_segments[i];
    ...
    void load_and_run_elf(elf_file &elf,
        ...
        if (explicit_use_interp) { // load the interpreter and use it
            ...
            for (size_t i = 0; i < elf.number_of_program_headers(); i++) {
                ...
                elf_file interp = elf_file(parse_elf_from_raw_file<CLASS>(open_raw_file(interp_path.c_str())));
...
public:
    void load_and_run_file(raw_file &&file) override {
        elf_file elf(parse_elf_from_raw_file<CLASS>(std::move(file)));
