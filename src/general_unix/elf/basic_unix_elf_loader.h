...
#define unix_elf_jump_entry_signature(name, id) void _ ##name## _elf_jump_entry_signature_ ##id (volatile void *exec_entry, volatile void *stack_entry)
...
#define RAND_ENDPOINT "/dev/random"
    ...
    basic_unix_elf_loader(const string &unixSpecificPlatform, const size_t unixSpecificPageSz,
                          void (*jumpEntry)(volatile void *, volatile void *)) : unix_specific_platform(
            unixSpecificPlatform), unix_specific_page_sz(unixSpecificPageSz), jump_signature(jumpEntry) {}

    // unix-specific distro properties
private:
    const string unix_specific_platform;
    const size_t unix_specific_page_sz;

    // jump entry signature
private:
    unix_elf_jump_entry_signature_t jump_signature;
public:
    void setJumpSignature(unix_elf_jump_entry_signature_t jumpSignature) {
        jump_signature = jumpSignature;
    }
